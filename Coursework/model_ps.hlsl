...
float4 calculateSpecular(float3 lightDirection, float3 normal, float3 viewVector, float4 specularColour, float specularPower)
{
    ...
    return specularColour.rgba * specularIntensity;
...
float4 calculateFinalLighting(int numberOfLights, float3 normal, float3 worldPosition, float4 specularMap, float4 lightViewPos[24]) // calculates all scene lighting
{
    ...
    float distance,
     attenuation;
    float2 pTexCoord;
    float4 specular = float4(specularMap.r, specularMap.r, specularMap.r, 1.0f);
                    ...
                    // Has depth map data
                        if (!isInShadow(depthMaps[shadowIndex], pTexCoord, lightViewPos[shadowIndex], 0.005f))
                        {
                         // is NOT in shadow, therefore light
                            ...
                            lightColour[i].rgba += specular.rgba * float4(attenuation, attenuation, attenuation, 1.0f);
